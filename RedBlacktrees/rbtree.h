/*******************************************************************************
 * Name        : rbtree.h
 * Author      : Michael Buzzetta Jorge Ponce
 * Version     : 1.0
 * Date        : Nov 13, 2023
 * Description : Implementation of red-black tree.
 * Pledge      : I pledge my honor that I have abided by the stevens honor system
 ******************************************************************************/
 //recieved help from CA Rishabh Wadhawan and Sarang Hadagali and tutor
#ifndef RBTREE_H_
#define RBTREE_H_
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) 
    {
        ...
        while(x != nullptr)
        {
            ...
            else if(key==x->key())
            {
                ...
                output <<"Attempt to insert duplicate key '" << key << "'.";
                delete z;
                throw tree_exception (output.str());
            }
            else
            {
                x=x->right;
...
private:
    ...
    int diameter(Node<K, V> *node) const 
    {
        ...
        */
        ...
        {
            int heightLeftBranch = height(node->left);
            int heightRightBranch = height(node->right);
            int overall=std::max(diameter(node->left), diameter(node->right));
            return std::max(heightLeftBranch+heightRightBranch+2, overall);
        }
    }
